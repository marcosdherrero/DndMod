...
public class FluidStackRenderer extends IIngredientRenderer<FluidStack> {
    ...
    public FluidStackRenderer() {
        this(FluidStack.convertDropletsToMb(FluidConstants.BUCKET),
                TooltipMode.SHOW_AMOUNT_AND_CAPACITY, 16, 16);
    }
     ...
     */
    public void drawFluid(MatrixStack matrixStack, FluidStack fluid,
                          int x, int y, int width, int height, long maxCapacity) {
        ...
        RenderSystem.setShaderTexture(
                0,
                FluidRenderHandlerRegistry.INSTANCE.get(fluid.getFluidVariant().getFluid())
                        .getFluidSprites(
                                MinecraftClient.getInstance().world,
                                null,
                                fluid.getFluidVariant().getFluid().getDefaultState())
                        [0].getId());
    }
    ...
    public List<Text> getTooltip(FluidStack fluidStack, TooltipContext tooltipFlag) {
        ...
        MutableText displayName = Text.translatable(
                "block." + Registry.FLUID.getId(fluidStack.fluidVariant.getFluid()).toTranslationKey());
        tooltip.add(displayName);

        long amount = fluidStack.getAmount();

        if (tooltipMode == TooltipMode.SHOW_AMOUNT_AND_CAPACITY) {
            MutableText amountString = Text.translatable(
                    "dndmod.tooltip.liquid.amount.with.capacity", nf.format(amount), nf.format(capacityMb));
            tooltip.add(amountString.fillStyle(Style.EMPTY.withColor(Formatting.DARK_GRAY)));
        }
        else if (tooltipMode == TooltipMode.SHOW_AMOUNT) {
            MutableText amountString = Text.translatable(
                    "dndmod.tooltip.liquid.amount", nf.format(amount));
            tooltip.add(amountString.fillStyle(Style.EMPTY.withColor(Formatting.DARK_GRAY)));
        }
